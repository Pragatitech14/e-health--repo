...
public class DisplayServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<CertificateBean> dlist1 = null;
		...
		try{
			...
			if(str.equals("ViewReport")){
				...
				dlist = rd.DisplayDiseaseHistory(username_op);
				request.setAttribute("DiseaseHistory", dlist);
				CertificateBean cib = rd.DisplayCertificate(username_op);
				request.setAttribute("Current_Cer_Obj", cib);
				dlist1 = rd.DisplayCertificateHistory(username_op);
				request.setAttribute("CertificateHistory", dlist1);
